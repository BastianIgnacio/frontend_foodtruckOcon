...
import { toggleProductCarousel2Api } from '../api'
...
export default function CarouselPage2() {
  ...
  const handleToggle = async (product) => {
    try {
      await toggleProductCarousel2Api(product.id)
      dispatch(fetchProducts({ active_only: true }))
    } catch (e) {
      console.error(e)
    }
  ...
  const carouselCount = products.filter(p => p.show_in_carousel2).length
  ...
  return (
    <Box>
      <Box sx={{ mb: 3, display: 'flex', alignItems: 'center', gap: 2, flexWrap: 'wrap' }}>
        ...
        <Box>
          <Typography variant="h5" fontWeight={700}>Carrusel de Productos 2</Typography>
          <Typography variant="body2" color="text.secondary">
            Selecciona los productos que se mostrarán en <strong>/productosVitrina2</strong>
          </Typography>
        </Box>
        <Chip
        ...
        {products.map(product => {
          ...
          return (
            <Grid item xs={12} sm={6} md={4} key={product.id}>
              <Card
                sx={{
                  opacity: product.show_in_carousel2 ? 1 : 0.6,
                  transition: 'all 0.2s',
                  border: '2px solid',
                  borderColor: product.show_in_carousel2 ? 'primary.main' : 'transparent',
                }}
              >
                <CardContent>
                  <Box sx={{ display: 'flex', alignItems: 'center', gap: 2 }}>
                    ...
                    <Switch
                      checked={!!product.show_in_carousel2}
